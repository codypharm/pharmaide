...
export default function TriageQueuePage() {
  ...
  const filteredEscalations = MOCK_ESCALATIONS.filter(e => {
    return severityFilter === "All" || e.severity === severityFilter.toLowerCase();
  });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3 relative">
            <button 
              ...
              className={`px-4 py-2 border rounded-xl font-semibold transition-all shadow-sm flex items-center gap-2 cursor-pointer ${severityFilter !== 'All' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}
            >
              <Filter size={16} />
              {severityFilter === 'All' ? 'Filter Queue' : `Severity: ${severityFilter}`}
            </button>
            {showFilterMenu && (
              <div className="absolute top-12 right-0 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 py-2 overflow-hidden animate-in fade-in zoom-in duration-200">
                <p className="px-4 py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50 mb-1">Filter by Severity</p>
                {["All", "Critical", "Warning"].map((level) => (
                  <button
                    key={level}
                    onClick={() => { setSeverityFilter(level); setShowFilterMenu(false); }}
                    className={`w-full px-4 py-2 text-sm text-left hover:bg-slate-50 transition-colors flex items-center justify-between font-medium ${severityFilter === level ? 'text-blue-600 bg-blue-50/50' : 'text-slate-600'}`}
                  >
                    {level}
                    {severityFilter === level && <CheckCircle2 size={14} />}
                  </button>
                ))}
              </div>
            )}
                    ...
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <Link to={`/dashboard/patient/${escalation.id}`} className="font-mono text-sm font-bold text-blue-600 hover:underline">ID: {escalation.id}</Link>
                          <span
                    ...
                    <tr key={iv.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-4">
                        <div className="flex flex-col gap-0.5">
                          <Link to={`/dashboard/patient/${iv.id}`} className="font-mono text-sm font-bold text-blue-600 hover:underline">{iv.id}</Link>
                          <span
