...
export default function AdherenceHeatmapsPage() {
  ...
  const sortedData = [...MOCK_HEATMAP_DATA].sort((a, b) => {
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl font-semibold hover:bg-slate-50 transition-colors shadow-sm flex items-center gap-2 cursor-pointer">
              <Calendar size={16} />
              ...
              <div className="flex gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <select 
                    ...
                    className="bg-white border border-slate-200 rounded-md px-3 py-1.5 text-sm outline-none w-40 cursor-pointer focus:ring-2 focus:ring-blue-100 transition-all"
                  >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select 
                    ...
                    className="bg-white border border-slate-200 rounded-md px-3 py-1.5 text-sm outline-none w-48 cursor-pointer focus:ring-2 focus:ring-blue-100 transition-all"
                  >
                  ...
                  {sortedData.map((pt) => (
                    <div key={pt.id} className="flex items-center group">
                      <div className="w-36 shrink-0 pr-4 flex flex-col gap-0.5">
                        <span className={`font-mono text-sm font-bold transition-colors ${pt.isCritical ? 'text-red-600 group-hover:text-red-700' : 'text-slate-900 group-hover:text-blue-600'}`}>
                          {pt.id}
                        </span>
                      </div>
                      <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-100 group-hover:border-slate-200 transition-colors">
                        {pt.history.map((status, i) => (
                          <div 
                            key={i} 
                            className={`w-4 h-6 mx-0.5 rounded-sm hover:opacity-80 transition-all cursor-crosshair hover:scale-y-110 ${
